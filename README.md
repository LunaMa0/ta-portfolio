...
在 `项目清单.xlsx` 的“加载底图”列中按项目填写：首页封面或视频尚未显示时，以及详情页视频尚未加载时，会使用这里的设置。

- 留空：继续使用网站自带的 6 套默认渐变。
- 纯色：例如 `#d8d3c8`、`rgb(216, 211, 200)`、`hsl(42, 17%, 82%)`。
- 渐变：例如 `linear-gradient(135deg, #d9ddd5, #6f8076)`。
- 图片：例如 `assets/images/placeholders/project-01.jpg`，路径建议使用 `/`。

填写后保存并关闭表格，再双击 `更新网站数据.bat`，刷新页面即可。
